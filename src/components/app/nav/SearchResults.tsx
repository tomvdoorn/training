...
const SearchResults: React.FC<SearchResultsProps> = ({ results, onClose }) => {
  return (
    <div className="absolute top-full left-0 w-full bg-background border border-gray-200 rounded-md shadow-lg mt-1 z-50">
      {results.map((result) => (
        <Link
          key={result.id}
        ...
        >
          {result.type === 'user' ? (
            ...
            <NotepadTextDashed className="w-4 h-4 mr-2" />
          )}
